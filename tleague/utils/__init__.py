...
def read_config_dict(config_name):
    try:
        ...
        print(
            "successfully _read_config_dict_py_module {}".format(config_name),
            flush=True,
        )
        return cfg
    except:
    ...
    try:
        ...
        print(
            "successfully _read_config_dict_py_expression {}".format(config_name),
            flush=True,
        )
        return cfg
    except:
...
def import_module_or_data(import_path):
    try:
        ...
        print(
            "trying from module {} import data {}".format(maybe_module, maybe_data_name)
        )
  ...
  It kills ALL sc2 processes on the machine"""
    outputs = (
        subprocess.check_output(
            "ps -aux | grep StarCraft | grep -v grep | awk '{print $2}' ", shell=True
        )
        .decode()
        .strip("\n")
    )
    if outputs:
        ...
        subprocess.check_output(
            "kill -9 {}".format(" ".join(outputs.split("\n"))), shell=True
        )
