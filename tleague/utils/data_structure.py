...
from tpolicies.tp_utils import template_structure_from_gym_space
...
class PGData(DataStructure):
    def __init__(
        ...
        if version == "v1":  # neglogp/logits is one long vector
            ...
            neglogp_shape_dtype = map_gym_space_to_structure(
...
class PPOData(PGData):
    ...
    def __init__(
        ...
        super(PPOData, self).__init__(
            ob_space, ac_space, n_v, use_lstm, hs_len, distillation, version
        )
        self.fields.extend(["R", "V"])
        self.specs.extend([([n_v], np.float32), ([n_v], np.float32)])
...
class VtraceData(PGData):
    def __init__(
        ...
        super(VtraceData, self).__init__(
            ob_space, ac_space, n_v, use_lstm, hs_len, distillation, version
        )
        self.fields.extend(["r", "discount"])
        self.specs.extend([([n_v], np.float32), ([], np.float32)])
        self.templates.extend([None, None])
        super(PGData, self).__init__(self.fields, self.specs, self.templates)
...
class InfData(DataStructure):
    def __init__(
        self, ob_space, ac_space, use_self_fed_heads=True, use_lstm=False, hs_len=None
    ):
        shape_dtype = lambda x: (x.shape, x.dtype)
        _fields = ["X"]
