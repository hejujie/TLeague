...
class LeagueMgr(BaseLeagueMgr):
    ...
    def __init__(
        ...
        game_mgr_config["pgn_file"] = (
            game_mgr_config.get("pgn_file", None) or "example.pgn"
        )
        game_mgr_config["verbose"] = game_mgr_config.get("verbose", None) or verbose
    ...
    def _on_request_learner_task(self, learner_id):
        ...
        if learner_id not in self._learner_task_table.keys():
            ...
            if hasattr(hyperparam, "init_model_key"):
                if hyperparam.init_model_key is not None:
                    assert (
                        hyperparam.init_model_key in self.init_model_keys
            ...
            task = LearnerTask(model_key, hyperparam, parent_model_key=cur_model_key)
            logger.log(
                "_on_request_learner_task: new learner_id, with model key "
            ...
            task = LearnerTask(model_key, hyperparam, parent_model_key=parent_model_key)
        ...
        self.game_mgr.add_player(
            task.model_key, parent_p=task.parent_model_key, learner_id=learner_id
        )
        logger.log(
            "_on_request_learner_task: ",
    ...
    def _on_request_actor_task(self, actor_id, learner_id):
        actor_task = super(LeagueMgr, self)._on_request_actor_task(actor_id, learner_id)
        if not isinstance(actor_task, LeagueMgrErroMsg):
            assert isinstance(actor_task, ActorTask)
            self.game_mgr.start_match(
                actor_task.model_key1, actor_task.model_key2, actor_id
            )

        logger.log("_on_request_actor_task: %s" % str(actor_task), level=logger.DEBUG)
        return actor_task

    def _on_request_eval_actor_task(self, actor_id):
        logger.log(
            "get_eval_actor_task: actor_id:{}".format(str(actor_id)), level=logger.DEBUG
        )
    ...
    def _on_request_train_actor_task(self, actor_id, learner_id):
        ...
        if (
            learner_id in self._learner_task_table
            ...
            if isinstance(oppo_model_key, LeagueMgrErroMsg):
                logger.log(
                    f"get_opponent not ready: {oppo_model_key}", level=logger.WARN
                )
            ...
            if learner_id not in self._learner_task_table:
                logger.log(
                    "learner_id({}) hasn't request_learner_task.".format(learner_id),
                    level=logger.WARN,
                )
    ...
    def _on_notify_actor_task_end(self, actor_id, match_result):
        # update match result
        m = match_result
        self.game_mgr.finish_match(
            m.model_key1, m.model_key2, m.outcome, m.info, actor_id
        )

    def _on_notify_learner_task_begin(self, learner_id, learner_task):
        super(LeagueMgr, self)._on_notify_learner_task_begin(learner_id, learner_task)
  ...
  over less physical learners (e.g., 8). """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(PARLeagueMgr, self).__init__(
            port, model_pool_addrs, mutable_hyperparam_type, **kwargs
        )
    ...
    def get_pseudo_id(self, learner_id):
        if learner_id not in self.id_map:
            ...
            logger.log(
                "learner {} begins training with pseude learner_id"
    ...
    def _on_notify_learner_task_begin(self, learner_id, learner_task):
        pseudo_id = self.get_pseudo_id(learner_id)
        return super(PARLeagueMgr, self)._on_notify_learner_task_begin(
            pseudo_id, learner_task
        )
    ...
    def _on_notify_learner_task_end(self, learner_id):
        ...
        self.id_map[learner_id] = self.cur_pseudo_id
        logger.log(
            "learner {} switches from pseudo learner_id {} to "
            "pseudo learner_id {}".format(
                learner_id, pseudo_id, self.id_map[learner_id]
            )
        )
        return super(PARLeagueMgr, self)._on_notify_learner_task_end(pseudo_id)

    def _on_request_actor_task(self, actor_id, learner_id):
        pseude_id = self.id_map[learner_id] if learner_id in self.id_map else learner_id
