...
from tleague.game_mgr.ae_game_mgrs import AEMatchMakingGameMgrV2
from tleague.game_mgr.ae_game_mgrs import IndivAdaEvoExploiter
from tleague.game_mgr.ae_game_mgrs import IndivCyclicExploiter
from tleague.game_mgr.ae_game_mgrs import IndivEvoExploiter
from tleague.game_mgr.ae_game_mgrs import IndivLeagueExploiter
from tleague.game_mgr.ae_game_mgrs import IndivMainAgent
from tleague.game_mgr.ae_game_mgrs import IndivMainExploiter
from tleague.game_mgr.ae_game_mgrs import IndivSKMainAgent
from tleague.game_mgr.ae_game_mgrs import NGUMatchMakingGameMgr
...
def check_save_load(gm, gm_cls):
    ...
    assert (
        np.sum(np.abs(gm.finished_match_counter - gm_new.finished_match_counter)) == 0
    )
    assert np.sum(np.abs(gm.sum_outcome - gm_new.sum_outcome)) == 0
    assert gm._population.keys() == gm_new._population.keys()
    if hasattr(gm, "_active_role_count") and hasattr(gm_new, "_active_role_count"):
...
def AEMatchMakingGameMgrV2_cyclic_exploiter_test():
    ...
    for i in range(14):
        ...
        print(
            "i {}, updated_current_p {}, parent_p {}".format(
