...
class PSROGameMgr(GameMgr):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logger = (
            Logger(dir=".", output_formats=[HOF(log_file)])
            if log_file
    ...
    def get_player(self, current_player):
        ...
        self.logger.log(
            "get_player(%s): %s" % (current_player, self.players[idx]),
            level=logger.DEBUG,
        )
    ...
    def get_opponent(self, player, dummy_hyperparam):
        ...
        self.logger.log(
            "get_opponent(%s): %s" % (player, self.players[idx]), level=logger.DEBUG
        )
    ...
    def get_eval_match(self):
        ...
        M = (
            self.finished_match_counter
            + self.finished_match_counter.transpose()
        ...
        self.logger.log(
            "get_eval_match(): %s, %s with %s matches finished."
            % (self.players[i], self.players[j], count),
            level=logger.DEBUG,
        )
...
class RefCountGameMgr(PSROGameMgr):
    ...
    def get_eval_match(self):
        ...
        n_player = len(self.players)
        M = (
            self.all_match_counter
            + self.all_match_counter.transpose()
            + np.diag([np.inf] * n_player)
        )
        idx = M.argmin()
        i, j = idx // n_player, idx % n_player
...
class PBTEloMatchMakingGameMgr(GameMgr):
  ...
  Allow mutation when accumulating sufficient lp (Learning Period)s """
    ...
    class Individual(object):
        ...
        def __str__(self):
            return (
                "Individual: elo_rating: {}, trained_lps: {}, "
                "total_trained_lps: {}, is_living: {}"
            ).format(self.elo_rating, self.trained_lps, self.total_lps, self.is_living)

    def __init__(
        self,
        pgn_file=None,
        log_file="elo.log",
        winrate_threshold=0.3,
        verbose=0,
        eligible_lps=1,
        **kwargs
    ):
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(PBTEloMatchMakingGameMgr, self).finish_match(
            rowp, colp, outcome, info, match_id
        )
        if outcome is None:  # actor task early abortion, ignore it
            self.logger.logp(
                "finish_match: outcome is None, ignore",
    ...
    means 'continue training'. """
        self.logger.log("cur player: {}".format(current_player), level=logger.DEBUG + 7)
        ...
        if not cur_individual.is_eligible():
            self.logger.log(
                "trained_lps: {} < {}".format(
                    cur_individual.trained_lps, self.eligible_lps
                ),
                level=logger.DEBUG + 7,
            )
            return None, False
        ...
        living_set = {
            player: individual
            for player, individual in self._population.items()
        ...
        winrate = elo_predict_winrate(cur_individual.elo_rating, candidate.elo_rating)
    ...
    def get_opponent(self, player, hyperparam):
     ...
     the WHOLE population, regardless of being dead or alive! """
        winrates = np.array(
            [
                elo_predict_winrate(
                    self._population[player].elo_rating,
    ...
    def _log_population(self, level):
        ...
        for p, i in self._population.items():
            self.logger.log("player: {}".format(p), "indiv: {}".format(i), level=level)
...
class PBTPSROGameMgr(GameMgr):
  ...
  optimization in parallel. See [1, 2, 3, 4].  """
    ...
    class Individual(object):
        ...
        def __str__(self):
            return (
                "Individual: trained_lps: {}, " "total_trained_lps: {}, is_living: {}"
            ).format(self.trained_lps, self.total_lps, self.is_living)

    def __init__(
        self,
        pgn_file=None,
        log_file="pbt_psro.log",
        verbose=0,
        eligible_lps=2,
        **kwargs
    ):
    ...
    def get_player(self, current_player):
     ...
     contracted value); If small, choose a random one from the living set. """
        ...
        self.logger.log(
            "get_player,",
            "cur player: {}".format(current_player),
            level=logger.DEBUG + 7,
        )
        # TODO: increment_lp() should be done at on_notify_learner_task_end
        self._population[current_player].increment_lp()
        ...
        self.logger.log(
            "get_player,", "living set payoff matrix A: ", level=logger.DEBUG + 7
        )
        ...
        if cur_d_big_enough:
            ...
            is_mutate = True
        self.logger.log(
            "get_player,", "new_player: {},".format(new_player), level=logger.DEBUG + 7
        )
        self._log_population(level=logger.DEBUG + 7)
        return new_player, is_mutate
    ...
    def _make_tasked_opponents(self, player):
        ...
        A, living_players = self._get_livingset_payoff_matrix()
        self.logger.log("make_tasked_opponents,", "A: ", level=logger.DEBUG + 7)
        self.logger.log(str(A), level=logger.DEBUG + 7)
        self.logger.log(
        ...
        self.logger.log(
            "make_tasked_opponents,",
            "ind_player: {},".format(ind),
            "vec_p: {}".format(vec_p.tolist()),
            level=logger.DEBUG + 7,
        )
        ...
        if np.all(vec_p < EPS):
            ...
            if random.random() < PROB_SELECT_SPECIFIC:
                ...
                p_opponents = [
                    player
                    for sign_payoff, player in zip(vec_p, living_players)
                    if sign_payoff > EPS
                ]
                self.logger.log(
                    "make_tasked_opponents,",
...
class ACMatchMakingGameMgr(GameMgr):
    ...
    class Individual(object):
        ...
        def __str__(self):
            _s_role = lambda i: (
                "AGENT" if i == ACMatchMakingGameMgr.Role.AGENT else "COMPETITOR"
            )
            _s_lrnobj = lambda i: (
                "IMPROVE" if i == ACMatchMakingGameMgr.LrnObj.IMPROVE else "EXPLOIT"
            )
            return (
                "Individual: role: {}, trained_lps: {}, total_trained_lps: {}, "
    ...
    def add_player(self, p, parent_p=None, **kwargs):
        ...
        self.logger.log(
            "all_match_counter: {}".format(self.all_match_counter), level=logger.DEBUG
        )
        self.logger.log("sum_outcome: {}".format(self.sum_outcome), level=logger.DEBUG)
        self.logger.log(
            "add_player:",
            "player: {},".format(p),
            "parent_player: {}".format(parent_p),
    ...
    def _add_to_population_as_agent(self, p, parent_p):
        ...
        individual = ACMatchMakingGameMgr.Individual(
            role=ACMatchMakingGameMgr.Role.AGENT, total_trained_lps=0
        )
        # inherit from parent individual...
        if parent_p is not None and parent_p in self._population:
            # ...the trained lps
            individual.total_trained_lps = self._population[parent_p].total_trained_lps
            # ...the competitors
            individual.prev_competitors = self._population[parent_p].prev_competitors
            individual.prev_avg_competitor_winrate = self._population[
                parent_p
            ].prev_avg_competitor_winrate
        ...
        self.logger.log(
            "_add_to_population_as_agent:",
            "player: {}, indv: {},".format(p, individual),
            "parent_player: {}, parent_indv: {},".format(parent_p, parent_i),
            level=logger.DEBUG + 7,
        )
        self._population[p] = individual
    ...
    def _make_lrn_obj(self, p, parent_p):
        ...
        if parent_p == "None:init_model" or parent_p is None:
            # random sampling
            lrn_obj = (
                ACMatchMakingGameMgr.LrnObj.IMPROVE
    ...
    def _add_to_population_as_competitor(self, p, parent_p):
        ...
        individual = ACMatchMakingGameMgr.Individual(
            role=ACMatchMakingGameMgr.Role.COMPETITOR, total_trained_lps=0
        )
        parent_i = None if parent_p is None else self._population[parent_p]
        self.logger.log(
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(ACMatchMakingGameMgr, self).finish_match(
            rowp, colp, outcome, info, match_id
        )
        self.logger.logp(
            "finish_match:",
    ...
    def _eligible_to_improve(self, current_player):
        ...
        if prev_competitors:
            ...
            new_winrate = self._collect_winrate_vec(
                current_player, list(prev_competitors.keys())
            )
        ...
        if competitors:
            ...
            winrate = self._collect_winrate_vec(
                current_player, list(competitors.keys())
            )
            avg_wr = winrate.mean()
    ...
    def _eligible_to_exploit(self, current_player):
        ...
        if agents:
            winrate = self._collect_winrate_vec(current_player, list(agents.keys()))
    ...
    def _collect_players_by_role(self, role):
        return {pp: ii for pp, ii in self._population.items() if ii.role == role}
    ...
    def _collect_winrate_vec(self, cur_player, other_players, minimal_matches=10):
        winrate, sum_outcome, match_count = self.get_one_vs_all_info(
            cur_player, other_players, minimal_matches
        )
        self.logger.log(
            "winrate: {},".format(winrate),
    ...
    def _log_population(self, level):
        ...
        for p, i in self._population.items():
            self.logger.log(
                "player: {},".format(p), "individual: {}".format(i), level=level
            )
...
class PBTACMatchMakingGameMgr(GameMgr):
    ...
    def _add_to_population_regular(self, p, parent_p):
        ...
        # Agent. Meanwhile, assign it a learning objective (by random sampling)
        lrn_obj = (
            PBTACMatchMakingGameMgr.LrnObj.IMPROVE
            if random.random() < 0.65
    ...
    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(PBTACMatchMakingGameMgr, self).finish_match(
            rowp, colp, outcome, info, match_id
        )
        self.logger.logp(
            "finish_match:",
    ...
    def _eligible_to_improve(self, current_player):
        is_eligible = True
        if (
            self._population[current_player].trained_lps
            < self.eligible_lps_by_lobj[PBTACMatchMakingGameMgr.LrnObj.IMPROVE - 1]
        ):
            is_eligible = False
            return is_eligible
        ...
        self._population[current_player].prev_competitors = competitors
        # update the metric - the prev_avg_competitor_winrate
        winrate = self._collect_winrate_vec(current_player, list(competitors.keys()))
        avg_wr = winrate.mean()
        self._population[current_player].prev_avg_competitor_winrate = avg_wr
    ...
    def _eligible_to_exploit(self, current_player):
        ...
        if (
            self._population[current_player].trained_lps
            < self.eligible_lps_by_lobj[PBTACMatchMakingGameMgr.LrnObj.EXPLOIT - 1]
        ):
            is_eligible = False
            return is_eligible
    ...
    def _collect_winrate_vec(self, cur_player, other_players):
        ...
        r_match_count = self.finished_match_counter[np.ix_(ind_row, ind_cols)].squeeze()
        # column vector
        ...
        c_match_count = self.finished_match_counter[np.ix_(ind_rows, ind_col)].squeeze()
