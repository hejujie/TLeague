...
from ..utils.tl_types import is_inherit
from ..utils.tl_types import LeagueMgrErroMsg
...
class Individual(object):
    ...
    def __str__(self):
        return (
            "sampled_as_oppo_count: {}, total_trained_lps: {}," "is_historical: {}"
        ).format(self.sampled_as_oppo_count, self.total_trained_lps, self.is_historical)
...
class IndivMainAgent(Individual):
    def __init__(
        ...
        **kwargs,
        ...
        super(IndivMainAgent, self).__init__(total_trained_lps)
    ...
    def decide_descendent(
        self, individuals, players, winrates, cur_player=None, coin_toss_reset=None
    ):
    ...
    def _pfsp_opponent(self, individuals, winrates):
        ...
        if historical_winrates:
            return np.random.choice(
                ...
                p=winrates_to_prob(historical_winrates, weighting="squared"),
            )
    ...
    def _forgotten_opponent(self, individuals, winrates):
        ...
        historical_individuals, historical_winrates = [], []
        for i, w in zip(individuals, winrates):
            if i.is_historical and isinstance(i, IndivMainAgent):
                historical_individuals.append(i)
                historical_winrates.append(w)
...
class IndivMainExploiter(Exploiter):
    def __init__(
        ...
        **kwargs,
        ...
        super(IndivMainExploiter, self).__init__(total_trained_lps)
    ...
    def decide_opponent(self, individuals, winrates, coin_toss=None):
        ...
        if historical_winrates:
            return np.random.choice(
                historical_individuals,
                p=winrates_to_prob(historical_winrates, weighting="variance"),
            )
  ...
  ml, hl, etc."""
    ...
    def find_best_by_pattern(self, individuals, players, active_ma_winrates):
        ...
        assert len(players) == len(
            active_ma_winrates
        ), "assert in find_best_by_pattern fails."
        ...
        for idx, player in enumerate(players):
            if (
                hasattr(individuals[idx], "_ee_pattern")
                and self._ee_pattern == individuals[idx]._ee_pattern
                and active_ma_winrates[idx] < best_winrate
            ):
                best_idx = idx
...
class IndivAdaEvoExploiter(IndivMainExploiter):
  ...
  same pattern."""
    ...
    def find_the_right_leaf_by_pattern(self, individuals, players, active_ma_winrates):
        ...
        for idx in leaf_idx:
            if (
                hasattr(individuals[idx], "_aee_pattern")
                and self._aee_pattern == individuals[idx]._aee_pattern
                and 0.5 < active_ma_winrates[idx] < 0.8
            ):
...
class IndivLeagueExploiter(Exploiter):
    def __init__(
        ...
        **kwargs,
        ...
        super(IndivLeagueExploiter, self).__init__(total_trained_lps)
        #
    ...
    def decide_descendent(
        ...
        if (
            self.total_trained_lps >= self.max_lps
    ...
    def decide_opponent(self, individuals, winrates, coin_toss=None):
        ...
        if historical_winrates:
            return np.random.choice(
                historical_individuals,
                p=winrates_to_prob(historical_winrates, weighting="linear_capped"),
            )
    ...
    def __str__(self):
        return "IndivLeagueExploiter, " + super(IndivLeagueExploiter, self).__str__()
...
class IndivCyclicExploiter(Exploiter):
    ...
    def _find_next_leaf(self, current_player, individuals, players):
        ...
        cyclic_leaves = [
            leaf for leaf in leaves if isinstance(leaf, IndivCyclicExploiter)
        ]
        ...
        historical_cyclic_leaves = [
            leaf for leaf in cyclic_leaves if leaf.is_historical
        ]
    ...
    def __str__(self):
        return "IndivCyclicExploiter, " + super(IndivCyclicExploiter, self).__str__()
  ...
  """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(AEMatchMakingGameMgr, self).__init__(
            pgn_file, stat_decay=stat_decay, verbose=verbose
        )
        self.logger = (
        ...
        assert len(self._lrn_id_list) == len(
            self._lrn_role_list
        ), """Length of
    ...
    def _log_add_player(self, p, parent_p, learner_id):
        ...
        self.logger.log(
            "all_match_counter:\n{}".format(self.all_match_counter), level=logger.DEBUG
        )
        self.logger.log("sum_outcome:\n{}".format(self.sum_outcome), level=logger.DEBUG)

    def finish_match(self, rowp, colp, outcome, info, match_id):
        super(AEMatchMakingGameMgr, self).finish_match(
    ...
    def get_player(self, current_player):
        ...
        if indiv.is_historical:
            self.logger.log(
                f"get_player for historical player {current_player}!", level=logger.WARN
            )
        indiv.increment_lp()
        ...
        winrates, _, _ = self.get_one_vs_all_info(
            cur_player=current_player, other_players=players, minimal_matches=5
        )

        if isinstance(indiv, IndivEvoExploiter) or isinstance(
            indiv, IndivAdaEvoExploiter
        ):
            active_individuals, active_players = zip(
                *[
                    (i, p)
                    for i, p in zip(individuals, players)
                    if not i.is_historical and isinstance(i, IndivMainAgent)
                ]
            )
            assert len(active_individuals) == 1, "only one active Main Agent"
    ...
    def get_opponent(self, player, hyperparam):
        ...
        if indiv.is_historical:
            self.logger.log(
                f"get_opponent for historical player {player}!", level=logger.WARN
            )

        players = list(self._population.keys())
        individuals = list(self._population.values())
        winrates, _, _ = self.get_one_vs_all_info(
            cur_player=player, other_players=players, minimal_matches=5
        )
        try:
            oppo_indiv = indiv.decide_opponent(
                individuals, winrates, self._dbg_coin_toss_oppo
            )
        except:
        ...
        self.logger.logp(
            "get_opponent," "player: {},".format(player),
...
class NGUMatchMakingGameMgr(AEMatchMakingGameMgr):
  ...
  """
    ...
    def _add_player_individual(self, p, parent_p, role_cls):
        if role_cls == IndivMainAgent:
            role_cls = IndivSKMainAgent
        super(NGUMatchMakingGameMgr, self)._add_player_individual(p, parent_p, role_cls)
