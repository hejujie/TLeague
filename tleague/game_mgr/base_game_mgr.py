...
class GameMgr(object):
    ...
    def load(self, checkpoint_dir):
        ...
        if len(missing_idx) > 0:
            print(
                "Players {} is missing in model pool".format(
        ...
        if self._use_player2order:
            self.player2order = dict([(p, i) for i, p in enumerate(self.players)])
        self.finished_matches = {
            (rp, cp): c
            for (rp, cp), c in finished_matches.items()
    ...
    """
        ...
        if n_players == 0:
            ...
            self.finished_match_counter = np.pad(
                self.finished_match_counter, (0, 1), "constant"
            )
            self.all_match_counter = np.pad(self.all_match_counter, (0, 1), "constant")
    ...
    """
        ...
        self.finished_match_counter[i, j] = _decay(self.finished_match_counter[i, j])
        self.finished_match_counter[j, i] = _decay(self.finished_match_counter[j, i])
