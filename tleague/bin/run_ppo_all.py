...
flags.DEFINE_string(
    "clust_spec_csv_path",
    "../sandbox/clust_spec_example.csv",
...
flags.DEFINE_boolean(
    "force_overwrite_remote", True, "Should force overwriting remote working folder?"
)
flags.DEFINE_string("tmux_sess", "tlea", "tmux session name for each machine")
flags.DEFINE_string(
    "local_worker_pre_cmd",
    "",
    "pre command for each LOCAL worker. " "e.g., activate python virutal env",
)
flags.DEFINE_string(
    "remote_worker_pre_cmd",
    "",
    "pre command for each REMOTE worker. " "e.g., activate python virutal env",
)
flags.DEFINE_string(
    "python_bin",
    "python",
    "python binary name or full path" "e.g., python, /usr/bin/python, python3",
)
flags.DEFINE_integer("n_process", 1, "number of parallel processes for connections.")
...
# actor
flags.DEFINE_integer("actor_update_model_freq", 32, "update model every n steps")
...
flags.DEFINE_integer(
    "actor_log_interval_steps", 51, "frequency of printing log in steps"
)
...
flags.DEFINE_integer(
    "lrn_pub_interval", 500, "freq of pub model to actors, in num of batch"
)
flags.DEFINE_integer("lrn_log_interval", 100, "freq of print log, in num of batch")
flags.DEFINE_integer(
    "lrn_total_timesteps",
    50000000,
...
flags.DEFINE_integer(
    "lrn_burn_in_timesteps",
    0,
    "total time steps for each learning period to burn in " "at beginning.",
)
flags.DEFINE_integer(
    "lrn_pull_worker_num", 2, "pull data worker number for each pull data socket"
)
flags.DEFINE_integer(
    "lrn_batch_worker_num", 4, "sample data worker number for each dataset"
)
flags.DEFINE_boolean("lrn_adv_normalize", True, "Whether normalize adv for ppo")
...
flags.DEFINE_string(
    "piecewise_fusion_schedule", "-1", "piecewise_fusion_schedule for policy_network"
)
flags.DEFINE_string(
    "piecewise_fusion_schedule_vf", "-1", "piecewise_fusion_schedule for value_network"
)
...
flags.DEFINE_string(
    ...
    "",
    "Config used for Mutable hyperparam. " "checkout tleague.hyperparam_mgr.configs.",
)
flags.DEFINE_string(
    "game_mgr_type",
    "RefCountGameMgr",
...
flags.DEFINE_string(
    "leagmgr_restore_checkpoint_dir",
    "",
    "checkpoint dir from which the tleague restores. ",
)
flags.DEFINE_string(
    "leagmgr_save_checkpoint_root",
    "",
    "checkpoint root dir to which the tleague saves. "
    "Will create sub folders during run.",
)
flags.DEFINE_integer(
    "leagmgr_save_interval_secs", 4800, "checkpoint saving frequency in seconds."
)
flags.DEFINE_integer(
    "leagmgr_verbose",
    11,
...
flags.DEFINE_boolean("mute_actor_msg", False, "Whether print req/rep of actors")
flags.DEFINE_integer(
    "pseudo_learner_num",
    0,
    "pseudo parallel run pseudo_learner_num learners " "with fewer real learners",
)
...
flags.DEFINE_boolean(
    "reboot_actor_on_failure", False, "Whether reboot when actor crushes."
)


def _learner_cmd(
    league_mgr_addr, model_pool_addrs, learner_ports, cuda_visible_devices
):
    ...
    return [
        ...
        FLAGS.lrn_rm_size,
        "--pub_interval",
        FLAGS.lrn_pub_interval,
        "--log_interval",
        FLAGS.lrn_log_interval,
        "--total_timesteps",
        FLAGS.lrn_total_timesteps,
        "--burn_in_timesteps",
        FLAGS.lrn_burn_in_timesteps,
        "--env",
        FLAGS.env,
        "--policy",
        FLAGS.policy,
        "--policy_config",
        make_quote_cmd(FLAGS.policy_config),
        ...
        FLAGS.n_v,
        "--rollout_length",
        FLAGS.lrn_rollout_length,
        "--rwd_shape" if FLAGS.lrn_rwd_shape else "--norwd_shape",
        "--merge_pi" if FLAGS.lrn_merge_pi else "--nomerge_pi",
        "--pull_worker_num",
        FLAGS.lrn_pull_worker_num,
        "--batch_worker_num",
        FLAGS.lrn_batch_worker_num,
        "--learner_config",
        make_quote_cmd(FLAGS.learner_config),
        "--type",
        "PPO",
    ]
...
def _actor_cmd(league_mgr_addr, model_pool_addrs, learner_addr):
    ...
    return [
        ...
        FLAGS.actor_verbose,
        "--log_interval_steps",
        FLAGS.actor_log_interval_steps,
        "--n_v",
        FLAGS.n_v,
        "--rwd_shape" if FLAGS.actor_rwd_shape else "--norwd_shape",
        "--reboot_on_failure"
        if FLAGS.reboot_actor_on_failure
...
def _league_mgr_cmd(model_pool_addrs, port):
    ...
    return [
        ...
        "--save_interval_secs",
        FLAGS.leagmgr_save_interval_secs,
        "--mute_actor_msg" if FLAGS.mute_actor_msg else "--nomute_actor_msg",
        "--verbose",
        FLAGS.leagmgr_verbose,
        "--pseudo_learner_num",
        FLAGS.pseudo_learner_num,
...
def main(_):
    ...
    act_addr_to_lrn = pair_actors_learners(
        actors=job_to_workers["actor"], learners=job_to_workers["learner"]
    )
    ...
    tmp_func = partial(
        ...
        zip_file_path,
        q_failed_workers,
        FLAGS.remote_working_folder,
        FLAGS.force_overwrite_remote,
        FLAGS.remote_worker_pre_cmd,
        FLAGS.local_worker_pre_cmd,
        FLAGS.python_bin,
        FLAGS.tmux_sess,
    )
    ...
    print("\nall commands sent. Succeeded: {}, Failed: {}".format(n_sucess, n_failure))
