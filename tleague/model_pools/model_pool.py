        ...
        while True:
            ...
            if msg == "model":  # get one Model
                ...
                self._rep_socket.send_pyobj(
                    ...
                    else self._model_pool[key]
                )
                logger.log(now() + "on msg read model,", "key: {}".format(key))
                ...
                self._rep_socket.send_pyobj(
                    {
                        k: (
                            ModelPoolErroMsg("Attribute not exits")
                ...
                key = self._rep_socket.recv_string()
                self._rep_socket.send_pyobj(
                    ModelPoolErroMsg("Key not exits")
                    if key not in self._model_pool
                    else None
                    if key not in self._learner_meta
