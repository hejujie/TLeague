...
from arena.interfaces.common import ActAsObs
from arena.interfaces.common import BoxTransformInt
from arena.interfaces.common import FrameStackInt
from arena.interfaces.common import MultiBinObsInt
...
def mk_pommerman_int_v1(**kwargs):
    ...
    from arena.interfaces.pommerman.obs_int import (
        BoardMapObs,
        AttrObsInt,
        PosObsInt,
        ActMaskObsInt,
...
def mk_pommerman_int_v2(**kwargs):
    from arena.interfaces.pommerman.obs_int_v2 import (
        BoardMapObs,
        AttrObsInt,
        ActMaskObsInt,
        RotateInt,
...
def create_pommerman_env(arena_id, env_config=None, inter_config=None):
    ...
    random_side = True if "random_side" not in env_config else env_config["random_side"]
    centralV = False if "centralV" not in env_config else env_config["centralV"]
    rotate = False if "rotate" not in env_config else env_config["rotate"]
    agent_list = None if "agent_list" not in env_config else env_config["agent_list"]
    rule_agents = [] if "rule_agents" not in env_config else env_config["rule_agents"]
...
def pommerman_env_space(arena_id, env_config=None, inter_config=None):
    env = create_pommerman_env(
        arena_id, env_config=env_config, inter_config=inter_config
    )
    env.reset()
