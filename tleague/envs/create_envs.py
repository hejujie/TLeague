...
def sc2_test(_):
    ...
    for i in range(2):
        ...
        print(f"episode: {i}")
        start_locations = [
            ob.game_info.start_raw.start_locations[0] for ob in env.unwrapped._obs
        ]
        base_pos = [
            u.pos
            for ob in env.unwrapped._obs
            for u in ob.observation.raw_data.units
            if u.alliance == 1 and u.unit_type == 86
        ]
        print("---------------")
        ...
        print("===============", flush=True)
        assert (
            start_locations[0].x == base_pos[1].x
            and start_locations[1].x == base_pos[0].x
        )
        for _ in range(10):
