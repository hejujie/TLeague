...
from arena.env.env_int_wrapper import SC2EnvIntWrapper
        ...
        for index in range(self.index + 1, index_now + 1):
            ...
            for i in range(1, self.len + 1):
                if all(order[index - 1] == self.target_order[i - 1]):
                    new_dist[i] = self.dist[i - 1] + self.pos_dist(
                        boc[index - 1], self.target_boc[i - 1]
                    )
                else:
...
class SC2ZergZStatVecReward(RewardWrapper):
  ...
  arbitrary value for player 1. Output vectorized reward. """
    ...
    def reset(self, **kwargs):
        ...
        self._potential_func = self._create_potential_func(
            obs[0], self.dict_space, self.version
        )
    ...
    def _create_potential_func(obs, dict_space, version):
        if version in ["v2", "v3"]:
            from timitate.lib6.zstat_utils import (
                BUILD_ORDER_OBJECT_CANDIDATES,
                RESEARCH_ABILITY_CANDIDATES,
            )

            split_indices = [
                len(BUILD_ORDER_OBJECT_CANDIDATES),
                -len(RESEARCH_ABILITY_CANDIDATES),
            ]
        ...
        def potential_func(raw_pb, ob, win):
            if use_zstat:
                ...
                d_order = [
                    -SeqLevDist.lev_dist(order, boc) / float(target_order.shape[0])
                ]
                # negative Hamming Distance as potential for binary unit count
                d_uc = [
                    -sum(u != target_u) / float(len(target_u))
                    for u, target_u in zip(uc, target_uc)
                ]
                if win:
                    d_order = [
                        -SeqLevDist.min_dist_possible() / float(target_order.shape[0])
                    ]
                    d_uc = [
                        -sum(u & ~target_u) / float(len(target_u))
                        for u, target_u in zip(uc, target_uc)
                ...
                if version == "v3":
                    d_order_bt = [
                        -SeqLevDist_bt.lev_dist(order_bt, boc_bt)
                        / float(target_order_bt.shape[0])
                    ]
                    if win:
                        d_order_bt = [
                            -SeqLevDist_bt.min_dist_possible()
                            / float(target_order_bt.shape[0])
                        ]
    ...
    def step(self, actions):
        ...
        if len(rwd) == 2:
            rwd[1] = (
                list(rwd[1]) + [0] * self.rwd_dim
                if isinstance(rwd[1], (list, tuple))
...
class OppoObsComponentsV2:
    def __init__(
        ...
        **kwargs,
        ...
        from timitate.lib6.pb2feature_converter import GlobalFeatMaker, ImmZStatMaker

        self._dict_space = dict_space
        self._components = [
            GlobalFeatMaker(game_version),
            ImmZStatMaker(
        ...
        if self._dict_space:
            ...
            for com in self._components:
                dict_features.update(
                    OrderedDict(
                        ["OPPO_" + k, v] for k, v in zip(com.tensor_names, com.make(pb))
                    )
                )
...
class SC2ZergOppoObsAsObs(OppoObsAsObs):
    ...
    def _expand_obs_space(self):
        ...
        if self._dict_space:
            ...
            self.observation_space.spaces[self._me_id] = spaces.Dict(
                OrderedDict(
                    list(space_old.spaces.items())
            ...
            if isinstance(self._attachment.space, spaces.Tuple):
                self.observation_space.spaces[self._me_id] = spaces.Tuple(
                    tuple(space_old.spaces) + tuple(self._attachment.space.spaces)
                )
            else:
                self.observation_space.spaces[self._me_id] = spaces.Tuple(
                    tuple(space_old.spaces) + (self._attachment.space,)
                )
        ...
        for ind, ob in enumerate(obs):
            if ob is not None:
                ...
                if isinstance(ob, dict) and set(self.required_keys) <= set(ob):
                    ...
                    print(
                        f"WARN: cannot find the fields {self.required_keys} for agent{ind}"
                    )
        for k in self.info:
            info[k] = self.info[k]
...
def create_sc2full_formal_env(
    ...
    if inter_fun in [make_sc2full_v8_interface]:
        ...
        env = make_sc2_base_env(
            n_players=2,
            step_mul=step_mul,
            version=version,
            screen_resolution=168,
            screen_ratio=0.905,
            camera_width_world_units=152,
            map_name=map_name,
            replay_dir=replay_dir,
    ...
    if astar_rwd:
        env = SC2ZergZStatVecReward(
            env, version=astar_rwd_version, compensate_win=compensate_win
        )
    if centralized_value:
        dict_space = (
            False if "dict_space" not in inter_config else inter_config["dict_space"]
        )
        if inter_fun in [make_sc2full_v8_interface]:
            zmaker_version = (
                "v4"
                if "zmaker_version" not in inter_config
                else inter_config["zmaker_version"]
            )
            max_bo_count = (
                50
                if "max_bo_count" not in inter_config
                else inter_config["max_bo_count"]
            )
            max_bobt_count = (
                20
                if "max_bobt_count" not in inter_config
                else inter_config["max_bobt_count"]
            )
            env = SC2ZergOppoObsAsObs(
                env,
                dict_space,
                OppoObsComponentsV2(
...
def create_sc2_env(arena_id, env_config=None, inter_config=None):
    ...
    difficulty = 0 if "difficulty" not in env_config else env_config["difficulty"]
    replay_dir = None if "replay_dir" not in env_config else env_config["replay_dir"]
    step_mul = None if "step_mul" not in env_config else env_config["step_mul"]
    map_name = (
        "KairosJunction" if "map_name" not in env_config else env_config["map_name"]
    )
    use_trt = False if "use_trt" not in env_config else env_config["use_trt"]
    astar_rwd_version = (
        "v3"
        if "astar_rwd_version" not in env_config
        else env_config["astar_rwd_version"]
    )
    compensate_win = (
        True if "compensate_win" not in env_config else env_config["compensate_win"]
    )
    ...
    game_core_config = (
        {} if "game_core_config" not in env_config else env_config["game_core_config"]
    )
    early_term = (
        True if "early_term" not in env_config else env_config["early_term"]
    )  # enabling by default
...
def sc2_env_space(arena_id, env_config=None, inter_config=None):
    ...
    centralized_value = (
        True
        if "centralized_value" not in env_config
        else env_config["centralized_value"]
    )
    import distutils

    version = "4.10.0"
    version_ge_414 = distutils.version.LooseVersion(
        version
    ...
    if arena_id in ["sc2full_formal8_dict"]:
        ...
        inter_config["dict_space"] = True
        inter = make_sc2full_v8_interface(**inter_config)
        ob_space = inter.observation_space
        ac_space = inter.action_space
        # TODO(pengsun): need interface_config here?? now it's hard-coding
        if centralized_value:
            com = OppoObsComponentsV2(game_version="4.10.0", lib="lib6", **inter_config)
            ob_space = spaces.Dict(
                OrderedDict(
                    list(ob_space.spaces.items()) + list(com.space.spaces.items())
                )
            )
    else:
