...
def _create_cig2107_track1_env(env_config, inter_config):
    ...
    num_players = 8 if "num_players" not in env_config else env_config["num_players"]
    num_bots = 0 if "num_bots" not in env_config else env_config["num_bots"]
    episode_timeout = (
        21000 if "episode_timeout" not in env_config else env_config["episode_timeout"]
    )
    is_window_visible = (
        False
        if "is_window_visible" not in env_config
        else env_config["is_window_visible"]
    )
    env = VizdoomMPEnv(
        config_path=path.join(
            path.dirname(arena.__file__), "utils/vizdoom/_scenarios/cig.cfg"
        ),
...
def vizdoom_env_space(arena_id, env_config=None, inter_config=None):
    ...
    if arena_id == "vizdoom_dogfight":
        ...
        ob_space = Box(shape=(168, 168, 6), low=0.0, high=255.0)
        ac_space = Discrete(6)
    elif arena_id == "vizdoom_cig2017_track2":
        raise NotImplementedError
    else:
