...
class PGActor(BaseActor):
  ...
  """
    ...
    def __init__(
        ...
        if self_infserver_addr is None:
            ...
            ds = InfData(
                ob_space,
                ac_space,
                policy_config["use_self_fed_heads"],
                self.rnn,
                nc.hs_len,
            )
        ...
        if self._enable_push:
            ...
            self._push_thread = Thread(
                target=self._push_data_to_learner, args=(self._data_queue,)
            )
        ...
        if self.distillation:
            ...
            if distill_infserver_addr is None:
                ...
                self.distill_agent = PGAgentGPU(
                    distill_infserver_addr, ds, nc.hs_len, compress
                )
    ...
    """
        ...
        logger.log(
            "pulling oppo hyperparam of model key {}".format(self.task.model_key2)
        )
        oppo_hyperparam = self._model_pool_apis.pull_attr(
            attr="hyperparam", key=self.task.model_key2
        )
        logger.log("Done pulling oppo hyperparam")
        oppo_inter_kwargs = {} if oppo_hyperparam is None else oppo_hyperparam.__dict__
        inter_kwargs = [self.task.hyperparam.__dict__] + [oppo_inter_kwargs] * (
            self.n_agents - 1
        )
        ...
        while True:
            ...
            predictions = self._parallel.run(
                (self._agent_pred, ob, i) for i, ob in enumerate(obs)
            )
            ...
            actions = [me_action] + [
                other_action for other_action in predictions[oppo_id:]
            ]
            ...
            if self._update_model_freq and self._steps % self._update_model_freq == 0:
                ...
                if (
                    self._enable_push
                    and self._model_pool_apis.pull_attr(
                        "freezetime", self.task.model_key1
                    )
                    is not None
                ):
    ...
    def _agent_pred(self, ob, i):
        ...
        if i == self._learning_agent_id:
            ...
            output = (
                self.agents[i].forward_squeezed(ob)
                if ob is not None
                else (None, 0, 0, 0)
            )
        else:
    ...
    def _should_update_model(self, model, model_key):
        if model is None or model_key != model.key:
            ...
            return (
                self._model_pool_apis.pull_attr("updatetime", model_key)
                > model.updatetime
            )
    ...
    The learning agent uses model1, all the other opponent(s) use model2 """
        logger.log(
            "entering _update_agents_model",
            "steps: {}".format(self._steps),
            level=logger.DEBUG + 5,
        )
        if self.self_infserver_addr is None and self._should_update_model(
            self.self_model, task.model_key1
        ):
    ...
    def _update_distill_agent_model(self):
        if self.distill_infserver_addr is not None:
            return
        logger.log(
            "entering _update_distill agent_model",
            f"steps: {self._steps}",
            level=logger.DEBUG + 5,
        )
    ...
    def _update_hyperparam(self, task):
        ...
        if self._enable_push:
            ...
            if hasattr(task.hyperparam, "reward_weights"):
                self._reward_weights = np.array(
                    task.hyperparam.reward_weights, dtype=np.float32
                )
    ...
    def _reward_shape(self, reward):
        ...
        assert (
            len(reward.shape) == 1 and reward.shape[0] == self._reward_weights.shape[-1]
        ), "reward {}, weights {}".format(str(reward), str(self._reward_weights))
