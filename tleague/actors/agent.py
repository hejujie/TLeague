...
class PGAgent(object):
  ...
  """
    ...
    def __init__(
        ...
        if use_gpu_id < 0:  # not using GPU
            ...
            self.net_out = policy.net_build_fun(
                self.inputs_ph, self.nc, scope=scope_name
            )
        else:
    ...
    def load_model(self, np_params):
        self.sess.run(
            self.params_assign_ops[: len(np_params)],
            feed_dict={ph: np_p for ph, np_p in zip(self.params_ph, np_params)},
        )
    ...
    def _feed_obs(self, obs):
        ...
        feed_dict = {
            ob_ph: [ob_np]
            for ob_ph, ob_np in zip(nest.flatten(self.inputs_ph.X), nest.flatten(obs))
        }
    ...
    def _forward(self, obs, fetches, action=None):
        ...
        if action is not None:
            for ac_ph, ac_np in zip(
                nest.flatten(self.inputs_ph.A), nest.flatten(action)
            ):
        ...
        self._state = (
            ...
            else _squeeze_batch_size_singleton_dim(ret["state"])
        )
        return ret
    ...
    def logits(self, obs, action=None):
        ...
        fetches = {
            "logits": nest.map_structure_up_to(
                self._ac_structure, lambda head: head.logits, heads
            )
        }
...
class PPOAgent(object):
   ...
   PGAgent instead."""
    ...
    def __init__(
        self, policy, ob_space, ac_space, n_v=1, scope_name="model", policy_config=None
    ):
        sess = tf.Session()
        self.ob_space = ob_space
    ...
    def _forward(self, obs):
        if self._state is not None:
            a, v, state, p, logits = self.policy.step(
                self._feed_obs(obs), [self._state]
            )
            self._last_state = self._state
            self._state = state
        else:
