...
from timitate.utils.const import MAP_PLAYABLE_AREA_DICT
from timitate.utils.const import MAP_PLAYABLE_SIZE_DICT
...
def _get_interface(map_name, game_core_config={}):
    crop = (
        game_core_config["crop_to_playable_area"]
        if "crop_to_playable_area" in game_core_config
        else False
    )
...
class ReplayExtractor(object):
    ...
    def extract(self):
        try:
            ...
            logger.log(
                "Extract replay[%s] player[%d] failed: %s"
        ...
        with run_config.start(version=self._version) as controller:
            ...
            while True:
                ...
                if last_obs is not None:
                    samples = self._replay_converter.convert(
                        pb=(last_obs, last_game_info), next_pb=(obs, game_info)
                    )
                    for data in samples:
                        yield data
...
class ReplayActor(object):
    def __init__(
        ...
        self.ds = ILData(
            ob_space, ac_space, self._use_policy, 1
        )  # hs_len does not matter
        ...
        while self.replay_task != "":
            ...
            for frame in extractor.extract():
                ...
                if self._use_policy and self._steps % self._update_model_freq == 0:
                    self._update_agent_model()
            logger.log(
                "Replay task [%s] done. %d frames sent to learner."
