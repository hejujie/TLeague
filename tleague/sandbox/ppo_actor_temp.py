...
class PPOAgent(object):
    ...
    def __init__(
        self, policy, ob_space, ac_space, n_v=1, action_mask=True, scope_name="model"
    ):
        sess = tf.Session()
...
class PPOActor(BaseActor):
  ...
  learner. """
     ...
     which will be sent to remote in a separate thread """
        ...
        while True:
            ...
            if self._update_model_freq and self._steps % self._update_model_freq == 0:
                if (
                    self._enable_push
                    and self._remote.pull_model_attr("freezetime", self.task.model_key1)
                    is not None
                ):
    ...
    def _update_agents_model(self, task):
        ...
        if (
            self.models[0] is None
            or task.model_key1 != self.models[0].key
            or (
                not self.models[0].is_freezed()
        ...
        if (
            self.models[1] is None
            or task.model_key2 != self.models[1].key
            or (
                not self.models[1].is_freezed()
    ...
    def _push_data(self, data_queue):
        ...
        logger.log(
            "entering _push_data_to_learner",
            "steps: {}".format(self._steps),
            level=logger.DEBUG + 5,
        )
        me_id = self._learning_agent_id  # short name
        ...
        while True:
            ...
            for _ in range(self._unroll_length):
                ...
                last_obs, actions, reward, info, done, other_vars = data_queue.get()
                value, state, neglogpac = other_vars
            if isinstance(last_obs[me_id], tuple) or isinstance(last_obs[me_id], list):
                mb_obs = tuple(
                    np.asarray(obs, dtype=obs[me_id].dtype) for obs in zip(*mb_obs)
                )
            else:
            ...
            for t in reversed(range(self._unroll_length)):
                next_values = (
                    value if t == self._unroll_length - 1 else mb_values[t + 1]
                )
                delta = (
                    mb_rewards[t]
                    + self._gamma * next_values * (1 - mb_dones[t])
                    - mb_values[t]
                )
                mb_advs[t] = last_gae_lam = (
                    delta + self._gamma * self._lam * (1 - mb_dones[t]) * last_gae_lam
                )
...
class PPOActorFixedOppo(PPOActor):
  ...
  opponent(s). Agent 0 viewed as learning agent. """
    ...
    def __init__(
        ...
        super(PPOActor, self).__init__(
            league_mgr_addr, model_pool_addrs, learner_addr=None
        )
        self.env = env
        self.env.reset()
        me_id = self._learning_agent_id  # short name
