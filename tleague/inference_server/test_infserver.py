...
def main(_):
    ...
    rnn = False if "use_lstm" not in policy_config else policy_config["use_lstm"]
    hs_len = (
        policy_config["hs_len"]
        if ("hs_len" in policy_config)
        else 2 * policy_config["nlstm"]
        if ("nlstm" in policy_config)
        else 128
    )
    ...
    if FLAGS.role == "Server":
        ...
        while True:
            ...
            cost = time.time() - t0
            print(
                "Predict {} samples costs {} seconds, fps {}.".format(
                    n, cost, n / cost
                ),
                flush=True,
            )
