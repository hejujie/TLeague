...
class InferDataServer(object):
    def __init__(
        self, port, batch_size, ds, batch_worker_num=4, use_gpu=True, compress=False
    ):
        ...
        dataset = (
            tf.data.Dataset.range(batch_worker_num)
            .apply(
                tf.contrib.data.parallel_interleave(
                    ...
                    sloppy=True,
                    buffer_output_elements=int(batch_size / batch_worker_num),
                )
            )
        ...
        while True:
            ...
            while not self._response_queue.empty():
                ...
                for data_id, output in zip(ids, outputs):
                    self._rep_socket.send_multipart(
                        [data_id, b"", pickle.dumps(output)]
                    )
...
class InfServer(object):
    def __init__(
        ...
        **kwargs
        ...
        self.net_out = policy.net_build_fun(
            self.data_server._batch_input, self.nc, scope="Inf_server"
        )
        # saving/loading ops
    ...
    def setup_fetches(self, outputs):
        def split_batch(template, tf_structure):
            ...
            return [
                nest.pack_sequence_as(template, flatten) for flatten in split_flatten
            ]

        if self.nc.use_self_fed_heads:
            a = nest.map_structure_up_to(
                self._ac_structure, lambda head: head.sam, self.net_out.self_fed_heads
            )
            ...
            logits = nest.map_structure_up_to(
                self._ac_structure,
            ...
            self.all_outputs = {
                ...
                if self.net_out.value_head is not None
                else [[]] * self.batch_size,
                "state": tf.split(self.net_out.S, self.batch_size)
                if self.net_out.S is not None
                else [[]] * self.batch_size,
            }
        else:
            logits = nest.map_structure_up_to(
    ...
    def run(self):
        while not self.data_server.ready:
            time.sleep(10)
            print(
                "Waiting at least {} actors to " "connect ...".format(self.batch_size),
                flush=True,
            )
        ...
        while True:
            ...
            if t0 > last_log_time + self._log_seconds:
                ...
                print(
                    "Process {} predicts {} samples costs {} seconds, fps {}".format(
                        pid, sam_num, cost, sam_num / cost
                    ),
                    flush=True,
                )
