...
class PGLearner(BaseLearner):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(PGLearner, self).__init__(
            league_mgr_addr, model_pool_addrs, learner_ports, learner_id
        )
     ...
     class."""
        ...
        self.trainer = tf.train.AdamOptimizer(
            learning_rate=self.LR, beta1=adam_beta1, beta2=adam_beta2, epsilon=adam_eps
        )
        ...
        (
            grads_and_vars,
            self.clip_grad_norm,
            self.nonclip_grad_norm,
        ) = self.clip_grads_vars(grads_and_vars, clip_vars, max_grad_norm)
        (
            grads_and_vars_vf,
            self.clip_grad_norm_vf,
            self.nonclip_grad_norm_vf,
        ) = self.clip_grads_vars(grads_and_vars_vf, clip_vars, max_grad_norm)
        ...
        self._build_ops()
        if has_hvd:
            barrier_op = hvd.allreduce(tf.Variable(0.0))
    ...
    def run(self):
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                logger.log(
                    "rank{}: done init_task and notify_task_begin...".format(self.rank)
                )
            else:
    ...
    def _init_const(
        ...
        self.rnn = (
            False if "use_lstm" not in policy_config else policy_config["use_lstm"]
        )
        self.hs_len = None
    ...
    def _build_ops(self):
        ...
        self.param_assign_ops = [
            p.assign(new_p) for p, new_p in zip(self.params, self.new_params)
        ]
        self.opt_params = self.trainer.variables()
        self.new_opt_params = [
            tf.placeholder(p.dtype, shape=p.get_shape()) for p in self.opt_params
        ]
    ...
    def _run_train_loop(self, nbatch):
        ...
        self.total_timesteps = getattr(
            self.task.hyperparam, "total_timesteps", self.total_timesteps
        )
        burn_in_timesteps = 0
        if self._need_burn_in:
            burn_in_timesteps = getattr(
                self.task.hyperparam, "burn_in_timesteps", self.burn_in_timesteps
            )
        ...
        for update in xrange(1, nupdates + 1):
            ...
            if (
                self.save_interval
                and (update % self.save_interval == 0 or update == 1)
                and logger.get_dir()
            ):
    ...
    def _init_task(self):
        task = self.task
        logger.log(
            "Period: {},".format(self._lrn_period_count), "Task: {}".format(str(task))
        )
        logger.log(
            "Continue training from model: {}. New model id: {}.".format(
    ...
    def clip_grads_vars(grads_and_vars, all_clip_vars, max_grad_norm):
        nonclip_grads_and_vars = [
            gv for gv in grads_and_vars if gv[1] not in all_clip_vars
        ]
        nonclip_grad_norm = tf.global_norm([grad for grad, _ in nonclip_grads_and_vars])
        clip_grads_and_vars = [gv for gv in grads_and_vars if gv[1] in all_clip_vars]
    ...
    def build_loss(self, model, input_data):
        ...
        if isinstance(self.ent_coef, list):
            assert len(entropy_list) == len(
                self.ent_coef
            ), "Lengths of ent and ent_coef mismatch."
        ...
        if self.distillation:
            ...
            if isinstance(self.distill_coef, list):
                assert len(distill_losses) == len(
                    self.distill_coef
                ), "Lengths of distill and distill_coef mismatch."
