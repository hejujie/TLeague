...
class ReplayMem(object):
    ...
    def sample_rollout(self):
        ...
        unroll = self._data_queue[
            i
        ]  # backup the unroll in case of overwritten by new unroll
...
class RMFromQueue(object):
    ...
    def __init__(
        ...
        self.pull_data_thread = Thread(
            target=self._pull_data, args=(pull_socket,), daemon=True
        )
        self.pull_data_thread.start()
        self.sample_rollout_thread = Thread(
            target=self._sample_rollout, args=(push_socket,), daemon=True
        )
    ...
    def _sample_rollout(self, push_socket):
        ...
        while True:
            ...
            for _ in range(self._rollout_per_batch):
                ...
                j = (
                    random.randint(0, self._rollout_per_unroll - 1)
...
class ReplayMemMP(ReplayMem):
    ...
    def __init__(
        ...
        super(ReplayMemMP, self).__init__(
            rm_size, unroll_length, batch_size, rollout_length, decode, minimal_sample
        )
        ...
        self._pull_sockets = [
            self._zmq_context.socket(zmq.PULL) for _ in range(n_process)
        ]
        self._output_eps = []
    ...
    def _start_subprocess(self):
        ...
        self._subps = [
            Process(
                target=self._run_sub_rm,
                args=(
                    self._input_ep,
                    output_ep,
                    maxl,
                    minl,
                    self._rollout_per_unroll,
                    self._rollout_length,
                    self._batch_size,
                    self._decode,
                ),
...
class ImpSampReplayMem(ReplayMem):
    ...
    def __init__(
        self, rm_size, unroll_length, batch_size, rollout_length, minimal_sample=None
    ):
        super(ImpSampReplayMem, self).__init__(
            rm_size,
            unroll_length,
            batch_size,
            rollout_length,
    ...
    def sample_rollout(self):
        ...
        p = np.sum(w, axis=1)
        j = np.random.choice(
            self._unroll_length // self._rollout_length, p=p / np.sum(p)
        )
