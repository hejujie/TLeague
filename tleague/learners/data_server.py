...
from tleague.learners.replay_memory import ImpSampReplayMem
from tleague.learners.replay_memory import ReplayMem
from tleague.learners.replay_memory import ReplayMemMP
...
class DataServer(object):
  ...
  Prepare batch data, prefetch using tf.data.Dataset"""
    ...
    def __init__(
        ...
        self._mk_rm(rm_size, unroll_length, batch_size, rollout_length, version, decode)

        self._pull_data_threads = []
        for i in range(pull_worker_num):
            self._pull_data_threads.append(Thread(target=self._pull_data, daemon=True))
    ...
    def _mk_rm(
        self, rm_size, unroll_length, batch_size, rollout_length, version, decode
    ):
        if version == "v2":
            self._replay_mem = ReplayMemMP(
                rm_size,
                unroll_length,
                batch_size,
                rollout_length,
            ...
            self._replay_mem = ReplayMem(
                rm_size, unroll_length, batch_size, rollout_length, decode=decode
            )
...
class ImDataServer(object):
    ...
    def __init__(
        ...
        self._pull_data_threads = [
            Thread(target=self._pull_data, daemon=True) for _ in range(pull_worker_num)
        ]
        self._train_rm = ImpSampReplayMem(
            rm_size, unroll_length, batch_size, rollout_length, min_train_sample_num
        )
        self._val_rm = ImpSampReplayMem(
            rm_size, unroll_length, batch_size, rollout_length, min_val_sample_num
        )
        ...
        train_dataset = tf.data.Dataset.range(train_generator_worker_num).apply(
            tf.contrib.data.parallel_interleave(
                lambda x: tf.data.Dataset.from_generator(
                    self._create_data_generator(self._train_rm), dtypes, shapes
                ).apply(tf.contrib.data.batch_and_drop_remainder(batch_size)),
        ...
        val_dataset = tf.data.Dataset.range(val_generator_worker_num).apply(
            tf.contrib.data.parallel_interleave(
                lambda x: tf.data.Dataset.from_generator(
                    self._create_data_generator(self._val_rm), dtypes, shapes
                ).apply(tf.contrib.data.batch_and_drop_remainder(batch_size)),
