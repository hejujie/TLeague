  ...
  """
    ...
    def __init__(
            ...
            if "use_xla" in policy_config and policy_config["use_xla"]:
                try:
                    ...
                    with tf.xla.experimental.jit_scope(True):
                        model = create_policy(
                            self.data_pool.train_batch_input, net_config
                        )
                except:
        ...
        nonclip_grads_and_vars = [
            (grad, var) for grad, var in grads_and_vars if var not in clip_vars
        ]
        ...
        def load_model(np_new_params):
            self._sess.run(
                self.param_assign_ops,
                feed_dict={p: np_p for p, np_p in zip(self.new_params, np_new_params)},
            )
        ...
        if after_loading_init_scope is not None:
            var_list = tf.get_collection(
                tf.GraphKeys.TRAINABLE_VARIABLES, scope=after_loading_init_scope
            )
        ...
        endpoints_aggregated = [
            _allreduce(tf.reduce_mean(l)) for l in model_val.endpoints.values()
        ]
        self._sess.graph.finalize()
        self._total_samples = lambda: [
            self.data_pool._num_train_samples,
            self.data_pool._num_val_samples,
        ]
        self._train_log_names = (
            ["loss"]
            + list(train_other_loss_names)
            + ["grad_norm", "clip_grad_norm", "param_norm"]
        )
        self._val_log_names = (
            ["loss"] + list(val_other_loss_names) + list(endpoints_names)
        )
        ...
        with logger.scoped_configure(
            dir="validation_log/rank{}".format(self.rank),
        ...
        for i in range(self._num_sgd_updates):
            ...
            if i % self._checkpoint_interval == 0:
                if self._checkpoints_dir is not None:
                    self._saver._save_model_checkpoint(
                        self._checkpoints_dir, "checkpoint_%s" % i
                    )
                if self._enable_validation:
                    ...
                    if self.rank == 0:
                        with logger.scoped_configure(logger=self.val_logger):
                            logger.logkvs(
                                {"n_update": i, "Elapsed Time": time.time() - t}
                            )
                    ...
                    logger.logkvs(
                        {
                            ...
                            for name, item in zip(
                                self._train_log_names, train_averaged_fetches
                            )
                        }
                    )
                    logger.dumpkvs()
    ...
    def _update_timing_logkvs(self, n_batches):
        tnow = time.time()
        consuming_fps = int(self._batch_size * n_batches / (tnow - self.tstart))
