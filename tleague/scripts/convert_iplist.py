...
FLAGS = flags.FLAGS
flags.DEFINE_string("input_path", "../sandbox/tail32", "input ip_list file path")
flags.DEFINE_string(
    "base_port",
    "9999",
    "base port for port1 & port2. " "It avoids port conflict on the same machine",
)
flags.DEFINE_string(
    "output_path",
    "../sandbox/b70_tail32.csv",
...
def read_ip_list(ip_list_path):
    with open(ip_list_path, "r") as fp:
        workers = [Worker(*line.strip().split(" ")) for line in fp.readlines()]
    return workers
