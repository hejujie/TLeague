...
def zip_containing_folder_relpath():
  ...
  """
    ...
    zip_file_path = os.path.join(
        tempfile.mkdtemp(), "{}.zip".format(containing_folder_basename)
    )
        ...
        for root, dirs, files in os.walk(containing_folder):
            for file in files:
                if _should_zip(root, file):
                    ...
                    arcname = (
                        containing_folder_basename + f_path[len(containing_folder) :]
                    )
...
def prepare_working_folder_remote(
    local_zip_path,
    ...
    with paramiko.SSHClient() as ssh:
        ...
        ssh.connect(
            ip, port=port, username=username, password=password, timeout=ssh_timeout
        )
        # make sure the target folder is ready
        sftp = ssh.open_sftp()
        is_remote_target_exist = _remote_target_exist(remote_target)
        print(
            "on remote: {}, overwrite_remote: {}, remote_target_exist: {}".format(
        ...
        if overwrite or not is_remote_target_exist:
            ...
            run_command_remote_ssh(
                ssh, "rm -r {} && rm {}".format(remote_target, remote_zip_path)
            )
            # copy local zip to remote
            print("copying {} to {}:{}...".format(local_zip_path, ip, remote_zip_path))
...
def run_cmd_remote(
    cmd, ip, port, username, password, info="", pre_cmd="", ssh_timeout=10
):
            ...
            if is_remote_ip(w.ip):
                if overwrite_remote:
                    prepare_working_folder_remote(
                        ...
                        ip=w.ip,
                        port=w.ssh_port,
                        username=w.ssh_username,
                        password=w.ssh_password,
                ...
                tmux_cmd = create_tmux_cmd(
                    ...
                    new_win_name=w.job,
                    cmd_exe=worker_cmd,
                    pre_cmd_exe=remote_worker_pre_cmd,
                )
                ...
                tmux_cmd = create_tmux_cmd(
                    ...
                    new_win_name=w.job,
                    cmd_exe=worker_cmd,
                    pre_cmd_exe=local_worker_pre_cmd,
                )
                run_cmd_local(
                    cmd=tmux_cmd, info=worker_cmd, pre_cmd=local_worker_pre_cmd
                )
                sleep(0.3)
